...
echo [3/4] Launching backend and celery worker...
REM The PCB layout engine is no longer a separate process. It lives in the API
REM as POST /pcb/compile (backend/pcb_engine/), so there is no port 8001 window.
...
echo   PCB Engine:  http://localhost:8000/pcb/compile  (inside the API)
